...
def fetch_record(user_input, session):
    with st.status("🔍 Searching in progress...", expanded=True) as status:
        extract_prompt = f"""
        You are a smart sales CRM assistant. 
        From the user's input, try undertand first what user is try to find and then extract:
        - COMPANY (if any i.e if any client's name or Company name)
        - EMAIL (if any)
        - FIRST_NAME and LAST_NAME (if any)
        - INDUSTRY (if mentioned)

        Return strictly a JSON object like, no other wrap up text:
        {{
          "COMPANY": "Adobe",
        ...
        llm_result = session.sql(f"""
            SELECT TRY_PARSE_JSON(SNOWFLAKE.CORTEX.COMPLETE('llama3-8b', $$ {extract_prompt} $$)) AS raw
        """).collect()[0][0]

        try:
            result = json.loads(llm_result)
        except:
            st.warning("Couldn’t extract enough info. Try rephrasing.")
            return

        filters = []
        if result.get("COMPANY"):
            filters.append(f"LOWER(COMPANY) LIKE '%{result['COMPANY'].lower()}%'")
        if result.get("EMAIL"):
            filters.append(f"LOWER(EMAIL) LIKE '%{result['EMAIL'].lower()}%'")
        if result.get("FIRST_NAME"):
            filters.append(f"LOWER(FIRST_NAME) LIKE '%{result['FIRST_NAME'].lower()}%'")
        if result.get("LAST_NAME"):
            filters.append(f"LOWER(LAST_NAME) LIKE '%{result['LAST_NAME'].lower()}%'")
        if result.get("INDUSTRY"):
            filters.append(f"LOWER(INDUSTRY) LIKE '%{result['INDUSTRY'].lower()}%'")
        # st.code(filters)

        if not filters:
            st.warning("No valid fields found to search. Try giving more context.")
            return

        query = f"""
            SELECT LEAD_ID, COMPANY, FIRST_NAME || ' ' || LAST_NAME AS CONTACT_NAME,
                   LEAD_SOURCE, RATING, BUSINESS_PAIN_POINTS, INDUSTRY, EMAIL, PHONE, CREATED_DATE
            FROM LEADS
        ...
        if results.empty:
            ...
            st.info("No matching lead records found.")
        else:
            status.update(label="record(s) found.", state="complete")
            st.subheader("🔎 Matching Lead Records")
    ...
    with st.status("Parsing reminder intent...", expanded=True) as status:
        ...
        insert_sql = f"""
            ...
            SELECT UUID_STRING(), '{reminder_type}', 'Pending', 'Normal', '{reminder_date}', '{description.replace("'", "''")}',
                   'avijit', '{lead_id}', TRUE, '{reminder_datetime}', TRUE, CURRENT_TIMESTAMP()
        """
    ...
    reminders = session.sql("""
        SELECT SUBJECT, DESCRIPTION, REMINDER_DATETIME,
               SNOWFLAKE.CORTEX.AI_COMPLETE(
                   'llama3-8b',
                   CONCAT(
                       'You are a CRM assistant. Convert this task into a short friendly reminder:/n',
                       'Subject: ', SUBJECT, '/n',
        ...
        LIMIT 5
    """).to_pandas()
    # reminders["REMINDER_MESSAGE"] = reminders["RAW_MESSAGE"].str.replace("\/n", " ", regex=False).str.replace('"', '')
        ...
        if lead_id_result:
            ...
            insert_deal_sql = f"""
                ...
                SELECT UUID_STRING(), '{lead_id}', 'Opportunity - {company}', {deal_amount}, 'Closed-Won', 0.4,
                       'New Business', '{close_date}', 'Pipeline', 'avijit', CURRENT_TIMESTAMP()
            """
...
db_schema=session.sql(f"""SELECT LISTAGG($1, '/n') FROM @CRM_SAMPLE_DB.PUBLIC.CRM_SCHEMA_STAGE/crm_schema_ss.sql
...
with st.sidebar:    
    ...
    st.markdown("<h4 style='text-align: left;'>👥 About SalesSense  :</h4>", unsafe_allow_html=True)
 
    st.markdown("""
     **"SalesSense"**
    is a Snowflake-powered AI assistant designed to help sales professionals manage leads, set reminders, and analyze sales data efficiently.
    """)
...
if st.button("🎙️Speak"):
    ...
    try:
        ...
        context_str = "/n".join([f"{entry['role']}: {entry['message']}" for entry in recent_context])
...
if st.button("## 🔔 Recommended Daily Follow-Ups"):
...
- lead update → when user wants to update lead contact info (e.g phone, email), 
   business pain points or convert a lead to close or win
- fetch record -> when the user wants to see or view a particular client's name or lead which is same as company, or any info like first naem , email info on display.
- sales strategy → when the user is asking for help , next action recommendations, sales ideas, business campaing ideas, 
preparing sale pitch deck for clients, asking advice, or suggestions about how to sell, qualify, or plan
...
# """).collect()[0][0]
    ...
    elif 'fetch record' in user_intent or 'fetch' in user_intent:
         fetch_record(safe_prompt, session)
